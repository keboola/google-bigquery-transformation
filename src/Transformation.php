...
class Transformation
{
    ...
    public function createManifestMetadata(
        ...
        foreach ($tableStructures as $tableStructure) {
            ...
            foreach ($tableDef->getColumnsDefinitions() as $column) {
                ...
                $primaryKey = in_array($column->getColumnName(), $tableStructure['primaryKey'], true);
     ...
     * @return array{'def': BigqueryTableDefinition, 'primaryKey': array}[]
     * @throws \Google\Cloud\Core\Exception\GoogleException
     * @throws \Keboola\Component\UserException
     */
    ...
    private function getTables(array $tables, bool $transformationFailed): array
    {
        ...
        foreach ($tables as $table) {
            try {
                $tableDef = $this->getDefinition($table['source']);
                $defs[] = [
                    'primaryKey' => $table['primary_key'] ?? $tableDef->getPrimaryKeysNames(),
                    'def' => $tableDef,
                ];
